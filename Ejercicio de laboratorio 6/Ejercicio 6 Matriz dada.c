...
void imprimirMatriz(int matriz[R][C], int n) {
    for (int i = 0; i < n; i++) {
        printf("| ");
        for (int j = 0; j < n; j++)
            printf("%d ", matriz[i][j]);
...
int minCostRecursivo(int cost[R][C], int n, int m)
{
    if (n < 0 || m < 0)
        return 1000000;
    else if (m == 0 && n == 0)
...
int main() {
    ...
    imprimirMatriz(matriz1, R);
    clock_t ini=clock();
    printf("Costo minimo: %d\n", mincost(matriz1, R - 1, R - 1));
    clock_t fin=clock();
    double tiempo = (double)(fin-ini)/CLOCKS_PER_SEC;
    printf("El Tiempo de costo minimo iterativo es : %f\n",tiempo);
    clock_t ini2=clock();
    printf("Costo minimo recursivo: %d\n", minCostRecursivo(matriz1, R - 1, R - 1));
    clock_t fin2=clock();
    double tiempo2=(double)(fin2-ini2)/CLOCKS_PER_SEC;
    printf("El Tiempo de costo minimo recursivo es : %f",tiempo2);
